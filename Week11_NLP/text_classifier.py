...
class TextClassifier:
    ...
    def __init__(self, stop_words_file=None, train_path=None, test_path=None):
        ...
        self._extract_stop_words()
        self.train_file_path = self._merge_csv_files(self.train_path)
        self.clf = None
        self.vocabulary = None
        self.train_data = self._get_train_data(self.train_file_path)
        self.test_data = self._get_test_data(test_path)

    def _extract_stop_words(self):
    ...
    def _merge_csv_files(self, input_dir=None, merged_csv='imdb_tr.csv'):
    ...
    @staticmethod
    def _load_csv_to_df(file_path=None, encoding="utf-8"):
        return pd.read_csv(file_path, encoding=encoding)

    def _get_train_data(self, file_path='imdb_tr.csv', encoding='utf-8'):
        df = self._load_csv_to_df(file_path, encoding="utf-8")
        return df

    def _get_test_data(self, file_path='imdb_te.csv', encoding='ISO-8859-1'):
        df = self._load_csv_to_df(file_path, encoding=encoding)
        df['text'] = df['text'].apply(self._format_text)
        return df
    ...
    def train_using_count(self, n_gram=1):
        ...
        self._apply_sgd_classifier(count_vectorizer)

    def train_using_tfidf(self, n_gram=1):
        tfidf_vectorizer = TfidfVectorizer(ngram_range=(1, n_gram),
                                           stop_words=self.stop_words_list)
        return self._apply_sgd_classifier(tfidf_vectorizer)
